...
const typeParsers = {
    ...
    'issues': issues => ({ issue: parseInt(issues, 10) }),
    'tree': branch => ({ branch }),
...
function startBookmarkDialog() {
    ...
    let bookmarkInput = '<form action=\\"#\\" id=\\"bookmark-input\\" onsubmit=\\"addBookmark(document.getElementById(' + bookmarkLink + ').value);return false;\\"><input id=\\"bookmark-link\\" placeholder=\\"Enter your link here...\\" /><button type=\\"submit\\">Add</button></form>'
...
async function parseGitLabUrl(link) {
    ...
    let result = await fetch('https://gitlab.com/api/v4/projects/' + encodeURIComponent(object.namespace + '/' + object.project) + '/issues/' + object.issue + '?access_token=' + access_token)
    let issuable = await result.json()
