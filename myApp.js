...
ipcMain.on('detail-page', (event, arg) => {
  ...
  if (arg.page == 'Project') {
    ...
    if (arg.page == 'Issues') {
      ...
      mb.window.webContents.executeJavaScript(
        'document.getElementById("detail-headline").innerHTML = "<span class=\\"name\\">' +
          arg.page +
...
ipcMain.on('sub-detail-page', (event, arg) => {
  ...
  if (arg.page == 'Issues') {
    ...
    let issuesSortSelect =
      ...
      '\\" class=\\"custom-option-label\\">Sort by due date</label></div></div>';
    mb.window.webContents.executeJavaScript(
      'document.getElementById("sub-detail-headline").innerHTML = "<span class=\\"name\\">' +
        arg.page +
        '</span><div class=\\"filter-sort\\">' +
        issuesQuerySelect +
        issuesStateSelect +
...
ipcMain.on('change-commit', (event, arg) => {
  ...
  mb.window.webContents.executeJavaScript(
    'document.getElementById("pipeline").innerHTML = "<div class=\\"commit empty\\"><div class=\\"commit-information\\"><div class=\\"commit-name skeleton\\"></div><div class=\\"commit-details skeleton\\"></div></div><div id=\\"project-name\\"></div></div>"',
  );
...
ipcMain.on('change-project-commit', (event, arg) => {
  ...
  mb.window.webContents.executeJavaScript(
    'document.getElementById("project-pipeline").innerHTML = "<div class=\\"commit empty\\"><div class=\\"commit-information\\"><div class=\\"commit-name skeleton\\"></div><div class=\\"commit-details skeleton\\"></div></div><div id=\\"project-name\\"></div></div>"',
  );
...
function openSettingsPage() {
  ...
  let darkString = "'dark'";
  mb.window.webContents.executeJavaScript(
    'document.getElementById("detail-headline").innerHTML = "<span class=\\"name\\">Theme</span>"',
  );
  let settingsString = '';
  let theme =
  ...
  if (store.user_id && store.username) {
    ...
    if (projects && projects.length > 0) {
      for (let project of projects) {
        favoriteProjects +=
          '<li><svg xmlns=\\"http://www.w3.org/2000/svg\\"><path fill-rule=\\"evenodd\\" clip-rule=\\"evenodd\\" d=\\"M2 13.122a1 1 0 00.741.966l7 1.876A1 1 0 0011 14.998V14h2a1 1 0 001-1V3a1 1 0 00-1-1h-2v-.994A1 1 0 009.741.04l-7 1.876A1 1 0 002 2.882v10.24zM9 2.31v11.384l-5-1.34V3.65l5-1.34zM11 12V4h1v8h-1z\\" class=\\"icon\\"/></svg><div class=\\"name-with-namespace\\"><span>' +
  ...
  mb.window.webContents.executeJavaScript(
    'document.getElementById("detail-content").innerHTML = "' + settingsString + '</div>"',
  );
  mb.window.webContents.executeJavaScript(
...
function saveUser(code, url = store.host) {
  ...
  fetch(url + '/api/v4/user?access_token=' + temp_access_token)
    .then((result) => {
      return result.json();
    })
    .then((result) => {
...
function getUser() {
  if (lastUserExecutionFinished && lastUserExecution + delay < Date.now()) {
    ...
    fetch(store.host + '/api/v4/user?access_token=' + store.access_token)
      .then((result) => {
        return result.json();
      })
      .then((user) => {
        if (user && !user.error) {
          ...
          let userString =
            ...
            '</span></div></a>';
          mb.window.webContents.executeJavaScript(
            'document.getElementById("user").innerHTML = "' + userString + '"',
          );
...
async function getUsersPlan() {
  fetch(store.host + '/api/v4/namespaces?access_token=' + store.access_token)
    .then((result) => {
      return result.json();
    })
    .then((namespaces) => {
...
function getLastEvent() {
  if (recentCommits && recentCommits.length > 0) {
    fetch(
      (url =
        store.host + '/api/v4/events?action=pushed&per_page=1&access_token=' + store.access_token),
    )
      .then((result) => {
        return result.json();
      })
...
function getLastTodo() {
  fetch(store.host + '/api/v4/todos?per_page=1&access_token=' + store.access_token)
    .then((result) => {
      return result.json();
    })
    .then((todos) => {
      ...
      if (lastTodoId != todo.id) {
        if (lastTodoId != -1 && Date.parse(todo.created_at) > Date.now() - 20000) {
          let todoNotification = new Notification({
            title: todo.body,
...
function getLastCommits(count = 20) {
  if (lastLastCommitsExecutionFinished && lastLastCommitsExecution + delay < Date.now()) {
    ...
    fetch(
      (url =
        store.host +
        '/api/v4/events?action=pushed&per_page=' +
        count +
        '&access_token=' +
        store.access_token),
    )
      ...
      .then((commits) => {
        if (commits && commits.length > 0) {
          ...
          if (committedArray && committedArray.length > 0) {
            ...
            getCommitDetails(
              committedArray[0].project_id,
              committedArray[0].push_data.commit_to,
              1,
            );
          } else {
            mb.window.webContents.executeJavaScript(
            ...
            mb.window.webContents.executeJavaScript(
              'document.getElementById("pipeline").innerHTML = "<p class=\\"no-results\\">You haven&#039;t pushed any commits yet.</p>"',
            );
          }
        } else {
          mb.window.webContents.executeJavaScript(
      ...
      if (commits && commits.length > 0) {
        ...
        fetch(
          ...
          .then((commit) => {
            ...
            mb.window.webContents.executeJavaScript(
              'document.getElementById("detail-headline").innerHTML = "' + pagination + '"',
            );
            mb.window.webContents.executeJavaScript(
        ...
        mb.window.webContents.executeJavaScript(
          'document.getElementById("project-commits-pagination").innerHTML = "<span class=\\"name\\">Commits</span>"',
        );
        mb.window.webContents.executeJavaScript(
          'document.getElementById("project-pipeline").innerHTML = "<p class=\\"no-results\\">No commits pushed yet.</p>"',
        );
      }
    });
}
      ...
      if (!projectArray.includes(commit.project_id)) {
        ...
        let result = await fetch(
          store.host +
            '/api/v4/projects/' +
            commit.project_id +
            '/pipelines?status=running&username=' +
            store.username +
            '&per_page=1&page=1&access_token=' +
            store.access_token,
        );
...
function getCommitDetails(project_id, sha, index) {
  ...
  fetch(store.host + '/api/v4/projects/' + project_id + '?access_token=' + store.access_token)
    .then((result) => {
      return result.json();
    })
    .then((project) => {
...
async function getMoreRecentlyVisited() {
  ...
  await BrowserHistory.getAllHistory(14320).then(async (history) => {
    ...
    let i = 0;
    mb.window.webContents.executeJavaScript(
      'document.getElementById("detail-headline").innerHTML = "<input id=\\"recentSearch\\" type=\\"text\\" onkeyup=\\"searchRecent(this)\\" placeholder=\\"Search...\\" />"',
    );
    let previousDate = 0;
    for (let j = 0; j < item.length; j++) {
      if (
    ...
    recentlyVisitedString += '</ul>';
    mb.window.webContents.executeJavaScript(
      'document.getElementById("detail-content").innerHTML = "' + recentlyVisitedString + '"',
    );
  });
}
...
function searchRecentlyVisited(searchterm) {
  ...
  foundString += '</ul>';
  mb.window.webContents.executeJavaScript(
    'document.getElementById("detail-content").innerHTML = "' + foundString + '"',
  );
}
...
async function getUsersProjects() {
  let result = await fetch(
    store.host +
      '/api/v4/users/' +
      store.user_id +
      '/starred_projects?min_access_level=30&per_page=' +
      numberOfFavoriteProjects +
      '&order_by=updated_at&access_token=' +
      store.access_token,
  );
...
function displayUsersProjects() {
  ...
  if (projects && projects.length > 0) {
    ...
    let chevron =
      '<svg class=\\"chevron\\" xmlns=\\"http://www.w3.org/2000/svg\\" viewBox=\\"0 0 16 16\\"><path class=\\"icon\\" fill-rule=\\"evenodd\\" d=\\"M5.29289,3.70711 C4.90237,3.31658 4.90237,2.68342 5.29289,2.29289 C5.68342,1.90237 6.31658,1.90237 6.70711,2.29289 L11.7071,7.29289 C12.0976,7.68342 12.0976,8.31658 11.7071,8.70711 L6.70711,13.7071 C6.31658,14.0976 5.68342,14.0976 5.29289,13.7071 C4.90237,13.3166 4.90237,12.6834 5.29289,12.2929 L9.58579,8 L5.29289,3.70711 Z\\" /></svg>';
  ...
  mb.window.webContents.executeJavaScript(
    'document.getElementById("projects").innerHTML = "' + favoriteProjectsString + '"',
  );
}
...
function getIssues(
  ...
  fetch(url)
    ...
    .then((issues) => {
      ...
      mb.window.webContents.executeJavaScript(
        'document.getElementById("' + id + '").innerHTML = "' + issuesString + '"',
      );
    });
}
...
function getMRs(
  ...
  fetch(url)
    ...
    .then((mrs) => {
      ...
      mb.window.webContents.executeJavaScript(
        'document.getElementById("' + id + '").innerHTML = "' + mrsString + '"',
      );
    });
}
...
function getTodos(
  ...
  fetch(url)
    ...
    .then((todos) => {
      ...
      mb.window.webContents.executeJavaScript(
        'document.getElementById("detail-content").innerHTML = "' + todosString + '"',
      );
    });
}
...
function displayPagination(keysetLinks, type) {
  ...
  if (keysetLinks.indexOf('rel="next"') != -1 || keysetLinks.indexOf('rel="prev"') != -1) {
    ...
    if (keysetLinks.indexOf('rel="prev"') != -1) {
      let prevLink = '';
      let icon =
        '<svg xmlns=\\"http://www.w3.org/2000/svg\\" viewBox=\\"0 0 16 16\\"><path class=\\"icon\\" fill-rule=\\"evenodd\\" d=\\"M10.707085,3.70711 C11.097605,3.31658 11.097605,2.68342 10.707085,2.29289 C10.316555,1.90237 9.683395,1.90237 9.292865,2.29289 L4.292875,7.29289 C3.902375,7.68342 3.902375,8.31658 4.292875,8.70711 L9.292865,13.7071 C9.683395,14.0976 10.316555,14.0976 10.707085,13.7071 C11.097605,13.3166 11.097605,12.6834 10.707085,12.2929 L6.414185,8 L10.707085,3.70711 Z\\"/></svg>';
    ...
    if (keysetLinks.indexOf('rel="next"') != -1) {
      let nextLink = '';
      let icon =
        '<svg xmlns=\\"http://www.w3.org/2000/svg\\" viewBox=\\"0 0 16 16\\"><path class=\\"icon\\" fill-rule=\\"evenodd\\" d=\\"M5.29289,3.70711 C4.90237,3.31658 4.90237,2.68342 5.29289,2.29289 C5.68342,1.90237 6.31658,1.90237 6.70711,2.29289 L11.7071,7.29289 C12.0976,7.68342 12.0976,8.31658 11.7071,8.70711 L6.70711,13.7071 C6.31658,14.0976 5.68342,14.0976 5.29289,13.7071 C4.90237,13.3166 4.90237,12.6834 5.29289,12.2929 L9.58579,8 L5.29289,3.70711 Z\\"/></svg>';
      if (keysetLinks.indexOf('rel="prev"') != -1) {
        nextLink = escapeHtml(
...
function setupEmptyProjectPage() {
  ...
  mb.window.webContents.executeJavaScript(
    'document.getElementById("detail-content").innerHTML = "' + emptyPage + '"',
  );
}
...
function displayProjectPage(project) {
  ...
  mb.window.webContents.executeJavaScript(
    'document.getElementById("detail-header-content").innerHTML = "<div id=\\"project-detail-information\\">' +
      logo +
      '<span class=\\"project-name\\">' +
      escapeHtml(project.name) +
      '</span><span class=\\"project-namespace\\">' +
      escapeHtml(project.namespace.name) +
      '</span></div><div class=\\"detail-external-link\\"><a href=\\"' +
      project.web_url +
...
function startBookmarkDialog() {
  ...
  mb.window.webContents.executeJavaScript(
    'document.getElementById("add-bookmark-dialog").innerHTML = "' + bookmarkInput + '"',
  );
...
function startProjectDialog() {
  ...
  mb.window.webContents.executeJavaScript(
    'document.getElementById("add-project-dialog").innerHTML = "' + projectInput + '"',
  );
  mb.window.webContents.executeJavaScript('window.scrollBy(0, 14)');
  mb.window.webContents.executeJavaScript(
...
function displaySkeleton(count, pagination = false, id = 'detail-content') {
  ...
  skeletonString += '</ul>';
  mb.window.webContents.executeJavaScript(
    'document.getElementById("' + id + '").innerHTML = "' + skeletonString + '"',
  );
}
