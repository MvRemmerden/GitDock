...
const getHttpAgent = (custom_cert_path) => {
  if(custom_cert_path === undefined){
    return undefined;
  }
  const ca = readCertFileSync(custom_cert_path);
...
module.exports = {
  ...
  async get(what, options = {}, host = store.host) {
    if (!options || !options.access_token) {
      options = { ...options, access_token: store.access_token, custom_cert_path: store.custom_cert_path, };
    }
    const agent = getHttpAgent(options.custom_cert_path);
    return fetch(`${host}/api/v4/${what}?${serializeAndEscapeOptions(options)}`, { agent: agent}).then((res) =>
      res.json(),
    );
  },
  async parseUrl(link) {
